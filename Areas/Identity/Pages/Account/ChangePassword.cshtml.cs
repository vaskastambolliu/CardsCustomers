...
namespace CardsCustomers.Areas.Identity.Pages.Account
{
    public class ChangePasswordModel : PageModel
    {
        ...
        [BindProperty]
        public int idUserToModifie { get; set; }
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            try
            {
                idUserToModifie = Convert.ToInt32(HttpContext.Request.Query["id"]);
                UserAdminService useradminservice = new UserAdminService(_context);
                UserAdmin useradmin = new UserAdmin();
                useradmin = useradminservice.GetUserAdminById(idUserToModifie);
                var user = await _userManager.FindByEmailAsync(useradmin.Email);
